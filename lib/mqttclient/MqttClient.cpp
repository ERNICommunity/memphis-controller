...
MqttClient::MqttClient(const char* mqttServerDomain, unsigned short int mqttPort, MemphisWiFiClient* wifiClient, MqttClientAdapter* adapter)
: m_wifiClient(wifiClient)
, m_adapter(adapter)
, m_pubSubClient(new PubSubClient(mqttServerDomain, mqttPort, *(wifiClient->getClient())))
...
{
  if ((0 != m_wifiClient) && (0 != m_pubSubClient))
  {
    while (WL_CONNECTED != m_wifiClient->getStatus())
    {
      delay(200);
    }
